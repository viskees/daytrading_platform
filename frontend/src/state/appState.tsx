...
function ImagePasteDrop({
  files,
  setFiles,
  label = "Click or paste screenshot (Ctrl/Cmd+V)",
}: {
  files: File[];
        ...
        <div className="mt-2 flex flex-wrap gap-2">
          {files.map((f, i) => (
            <div key={i} className="relative">
              <img alt={f.name} src={URL.createObjectURL(f)} className="h-16 w-24 object-cover rounded-lg border" />
            </div>
          ))}
        </div>
      )}
...
type AppState = {
  // Core state
  risk: RiskPolicy;
  setRisk: React.Dispatch<React.SetStateAction<RiskPolicy>>;

  openTrades: Trade[];
  setOpenTrades: React.Dispatch<React.SetStateAction<Trade[]>>;
  ...
  usedDailyRiskPct: number;
  kpiError: string | null;

  // Derived budgets/guards
  openRisk$: number;
  realizedLoss$Today: number;
  baseEquityForBudget: number;
  displayUsedDailyRiskPct: number;
  usedPctOfBudget: number;
  ...
  getDayTrades: (isoDate: string) => Promise<any[]>;
  ...
  closing: Trade | null;
  setClosing: React.Dispatch<React.SetStateAction<Trade | null>>;

  editOpen: boolean;
  setEditOpen: React.Dispatch<React.SetStateAction<boolean>>;
  editing: any | null;
  setEditing: React.Dispatch<React.SetStateAction<any | null>>;

  onEditTrade: (t: any) => void;
  onSaveEdit: (vals: any) => Promise<void>;
};
...
export function AppStateProvider({ children }: { children: ReactNode }) {
  ...
  const perTradeRisk$ = (t: Trade) => {
    ...
    const perUnit =
      t.side === "LONG" ? Math.max(0, t.entryPrice - t.stopLoss) : Math.max(0, t.stopLoss - t.entryPrice);
  ...
  const perTradeCap$ = useMemo(
    () => (baseEquityForBudget > 0 ? baseEquityForBudget * (risk.maxRiskPerTradePct / 100) : 0),
    [baseEquityForBudget, risk.maxRiskPerTradePct]
  );

  const dailyBudget$ = useMemo(
    () => (baseEquityForBudget > 0 ? baseEquityForBudget * (risk.maxDailyLossPct / 100) : 0),
  ...
  const getMonthSummaries = useCallback(async (year: number, month0: number): Promise<DaySummary[]> => {
    ...
    // Prefer server-side daily P/L endpoint if available.
    try {
    ...
    try {
      ...
      for (const day of Object.keys(byDay)) {
        ...
        const rVals = trades.map((t: any) => Number(t.r ?? 0)).filter((x: number) => isFinite(x));
        const wins = rVals.filter((x: number) => x > 0).length;
        const lossRs = rVals.filter((x: number) => x < 0);
        const bestR = rVals.length ? Math.max(...rVals) : 0;
        const worstLossR = lossRs.length ? Math.min(...lossRs) : 0;
        const avgR = rVals.length ? rVals.reduce((a: number, b: number) => a + b, 0) / rVals.length : 0;
  ...
  const getDayTrades = useCallback(async (isoDate: string) => {
    try {
      const res = await apiFetch(`/journal/trades/?status=CLOSED&journal_day__date=${isoDate}&ordering=entry_time`);
      const payload = await res.json();
      const items: any[] = Array.isArray(payload) ? payload : payload.results ?? [];
      return items.map((t) => ({
        id: t.id,
  ...
  const refreshDashboard = useCallback(async () => {
    ...
    try {
      ...
      if (msg.toLowerCase().includes("throttled") || msg.includes("429")) {
        refreshGuard.current.nextOk = now + 60_000;
      }
    }
    ...
    try {
      ...
      setTotalEquity((prev) => {
        ...
        try {
          localStorage.setItem(LAST_EQUITY_KEY, String(val));
        } catch {}
        return val;
      });
    } catch (e: any) {
  ...
  // theme → server (debounced). We do NOT setTheme() here anymore (AppShell owns DOM theme)
  const debouncedSaveTheme = useMemo(
    () =>
      debounce((theme: "dark" | "light") => {
        apiSaveTheme(theme);
      }, 800),
    []
  );

  // After mount: dev sanity tests
  useEffect(() => {
    runDevTests();
  }, []);

  // After mount: fetch settings + trades + initial KPIs
  useEffect(() => {
    (async () => {
      try {
        const settings = await apiFetchUserSettings();
        if (settings) {
          // Do NOT override a user-chosen theme; AppShell controls DOM theme.
          // We still keep this behavior so server theme can be adopted when no local pref exists,
          // by calling saveTheme when needed. (No-op if you already set theme in AppShell.)
          if (!hasStoredTheme()) {
            // If you want to also auto-sync AppShell toggle, do it in AppShell instead.
            // Keeping this as a no-op here to avoid double sources of truth.
          }
  ...
  // Detect local date rollover and refresh once
  useEffect(() => {
      ...
      <CardContent className="p-4">
        ...
        <div className="grid grid-cols-3 gap-4 text-sm">
          <div>
            ...
            <div className="font-medium">{s.risk.maxRiskPerTradePct}%</div>
          </div>
          <div>
            <div className="text-muted-foreground">Max loss / day</div>
            <div className="font-medium">{s.risk.maxDailyLossPct}%</div>
          </div>
          <div>
            <div className="text-muted-foreground">Max trades / day</div>
            <div className="font-medium">{s.risk.maxTradesPerDay}</div>
          </div>
        </div>
        ...
        <div className="mt-4">
          <div className="flex justify-between text-xs mb-1">
            ...
            <span className="font-medium">
              {s.displayUsedDailyRiskPct.toFixed(2)}% / {s.risk.maxDailyLossPct}%
            </span>
          </div>
          <Progress value={s.usedPctOfBudget} />
          <div className="mt-1 text-[11px] text-muted-foreground">
            Open risk: ${s.openRisk$.toFixed(2)} · Realized loss: ${s.realizedLoss$Today.toFixed(2)} · Budget: $
            {(s.baseEquityForBudget * (s.risk.maxDailyLossPct / 100)).toFixed(2)}
          </div>
        </div>

        {s.kpiError && (
          <div className="mt-3 text-xs text-amber-600">
            {s.kpiError} &mdash; set your account/risk in{" "}
            <button className="underline" onClick={() => navigate("/app/risk")}>
              Risk Settings
            </button>
            .
          </div>
      ...
      <CardContent className="p-4">
        ...
        <ul className="grid grid-cols-3 gap-4 text-sm">
          <li>
            ...
            <div className="font-semibold">{s.todaysPL.toFixed(2)}</div>
          </li>
          <li>
            <div className="text-muted-foreground">P/L (total)</div>
            <div className="font-semibold">{s.totalPL.toFixed(2)}</div>
          </li>
          <li>
            <div className="text-muted-foreground">Total equity</div>
            <div className="font-semibold">{s.totalEquity.toFixed(2)}</div>
          </li>
        </ul>
    ...
    <Card>
      <CardContent className="p-4">
        ...
        <div className="grid grid-cols-3 lg:grid-cols-6 gap-4 text-sm">
          <Stat label="Trades" value={s.session?.trades ?? 0} />
          <Stat label="Win rate" value={`${(s.session?.win_rate ?? 0).toFixed(1)}%`} />
          <Stat label="Avg R" value={(s.session?.avg_r ?? 0).toFixed(2)} />
          <Stat label="Best R" value={(s.session?.best_r ?? 0).toFixed(2)} />
          <Stat label="Worst R" value={(s.session?.worst_r ?? 0).toFixed(2)} />
          <Stat label="Max DD" value={`${(s.session?.max_dd_pct ?? 0).toFixed(2)}%`} />
        </div>
        <p className="mt-3 text-xs text-muted-foreground">
          Source: <code>/api/journal/trades/status/today/</code>
        </p>
      </CardContent>
    </Card>
  );
    ...
    <Card>
      <CardContent className="p-4">
        <div className="flex items-center justify-between mb-2">
          ...
          <Button variant="outline" size="sm" onClick={() => s.setShowNew(true)}>
            New trade
          </Button>
        </div>
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {s.openTrades.map((t) => (
                <tr key={t.id} className="border-b last:border-none align-top">
                  ...
                  {(() => {
                    ...
                    const r = s.perTradeCap$ > 0 ? riskDollar / s.perTradeCap$ : 0;
                  ...
                  <td className="py-2 pr-2 space-y-2">
                    <div className="flex items-center gap-2">
                      <Button size="sm" variant="outline" onClick={() => s.onEditTrade(t)}>
                        Edit
                      </Button>
                      <Button size="sm" variant="outline" onClick={() => s.setClosing(t)}>
                        Close
                      </Button>
                    </div>
                  </td>
              ...
              {s.openTrades.length === 0 && (
                <tr>
                  <td colSpan={10} className="py-4 text-sm text-muted-foreground">
                    No open trades.
                  </td>
                </tr>
              )}
        ...
        <p className="mt-3 text-xs text-muted-foreground">Moves to the Journal section after closing.</p>
      </CardContent>
    </Card>
  );
}
...
export function DashboardPage() {
  ...
  return (
    <div className="space-y-4">
      ...
      <TradesCalendar
        refreshToken={s.calendarKick}
        dayStartEquity={s.dayStartEquity}
        maxDailyLossPct={s.risk.maxDailyLossPct}
        getMonthSummaries={s.getMonthSummaries}
        getDayTrades={s.getDayTrades}
      />
    </div>
  );
}
              ...
              <tbody>
                {MOCK_TICKERS.map((t) => (
                  <tr key={t.symbol} className="border-b last:border-none">
                    ...
                    <td className={`py-2 pr-2 ${t.changePct && t.changePct >= 0 ? "text-emerald-600" : "text-red-600"}`}>
...
/* =========================
   Risk Settings page (as-is, but route-safe)
   ========================= */
export function RiskSettingsPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (!d || !Number.isInteger((d as any).id)) throw new Error("Failed to ensure today's Journal Day");

      const id = Number((d as any).id);
      setJournalDayId(id);
      setDayStartEquity(Number((d as any).day_start_equity || 0));
      setEffectiveEquity(Number((d as any).effective_equity || 0));
      setAdjustmentsTotal(Number((d as any).adjustments_total || 0));
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div>
            ...
            <Input
              type="number"
              step="0.1"
              value={s.risk.maxRiskPerTradePct}
              onChange={(e) => s.setRisk({ ...s.risk, maxRiskPerTradePct: Number(e.target.value) })}
            />
          </div>
          <div>
            ...
            <Input
              type="number"
              step="0.1"
              value={s.risk.maxDailyLossPct}
              onChange={(e) => s.setRisk({ ...s.risk, maxDailyLossPct: Number(e.target.value) })}
            />
          </div>
          <div>
            ...
            <Input
              type="number"
              value={s.risk.maxTradesPerDay}
              onChange={(e) => s.setRisk({ ...s.risk, maxTradesPerDay: Number(e.target.value) })}
            />
          </div>
        </div>

        <div className="text-sm text-muted-foreground">
          Will POST to <code>/api/risk-policy</code>.
        </div>
            ...
            <>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 items-end">
                <label className="block">
                  ...
                  <Input type="number" step="0.01" value={dayStartEquity} onChange={(e) => setDayStartEquity(Number(e.target.value))} />
                </label>
              ...
              <div className="mt-5">
                ...
                <div className="grid grid-cols-1 md:grid-cols-5 gap-3 items-end mb-3">
                  <label className="block md:col-span-2">
                    ...
                    <Input type="number" step="0.01" value={adjAmount} onChange={(e) => setAdjAmount(Number(e.target.value))} />
                  </label>
...
export function AccountPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      setErr(null);
      try {
        const [meData, tfaStatus] = await Promise.all([fetchMe(), fetchTwoFAStatus().catch(() => null)]);
        if (cancelled) return;
        setMe(meData);
  ...
  return (
    <>
      {showQr && tfaConfigUrl && (
        <ModalShell title="Scan this code with your authenticator app" onClose={() => setShowQr(false)}>
          <form className="flex flex-col items-center gap-4" onSubmit={handleTfaVerify}>
            ...
            {tfaSecret && (
              <p className="text-xs text-muted-foreground text-center">
                ...
                <code className="px-1 py-0.5 rounded bg-neutral-900 text-[11px]">{tfaSecret}</code>
              </p>
            )}

            <div className="w-full max-w-xs space-y-2">
              <div className="text-xs text-muted-foreground">Voer nu de 6-cijferige code van je app in om 2FA te activeren.</div>
              <Input
                inputMode="numeric"
                pattern="\d*"
                maxLength={8}
            ...
            <div className="flex items-center gap-3">
              ...
              {tfaError && <div className="text-xs text-red-600 max-w-xs text-center">{tfaError}</div>}
            </div>
          </form>
        </ModalShell>
      )}
            ...
            <>
              <section className="space-y-2">
                ...
                <div className="text-sm">
                  ...
                  {me.last_login && (
                    <div className="text-xs text-muted-foreground">Last login: {new Date(me.last_login).toLocaleString()}</div>
                  )}
                </div>
              </section>
...
/* =========================
   Modals (New trade / Close trade / Editor)
   Render this ONCE somewhere under AppStateProvider.
   ========================= */
  ...
  const submit = async () => {
    ...
    try {
      ...
      if (guardsEnabled && exceedsPerTrade) {
        throw new Error(
          `This trade risks $${currentRisk$.toFixed(2)}, above your per-trade cap of $${perTradeCap$.toFixed(2)}.`
        );
      }
    ...
    <ModalShell title="New trade" onClose={onClose}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
        ...
        <div className="md:col-span-3">
          ...
          <div className="flex items-center gap-2">
            <Button size="sm" variant={entryEmotion === "NEUTRAL" ? "default" : "outline"} onClick={() => setEntryEmotion("NEUTRAL")}>
              Neutral
            </Button>
            <Button size="sm" variant={entryEmotion === "BIASED" ? "default" : "outline"} onClick={() => setEntryEmotion("BIASED")}>
              Biased
            </Button>
          </div>
          <div className="mt-2">
      ...
      <div className="mt-4 flex justify-end gap-2">
        ...
        <Button
          ...
          title={
            kpiError ??
            (guardsEnabled ? (exceedsPerTrade ? "Exceeds per-trade cap" : exceedsDaily ? "Exceeds daily budget" : "") : "")
          }
        >
          Create
        </Button>
      ...
      <div className="mt-2 text-[11px] text-muted-foreground">
        Trade risk: ${currentRisk$.toFixed(2)} · Per-trade cap: ${perTradeCap$.toFixed(2)} · Daily remaining: $
      ...
      {kpiError && (
        <div className="mt-2 text-xs text-amber-600">
          ...
          <button className="underline" onClick={onGoRisk}>
            Risk Settings
          </button>
          .
        </div>
...
function CloseTradeDialog({
  ...
  trade: Trade;
  onClosed: (id: string | number) => void;
  onClose: () => void;
}) {
    ...
    <ModalShell title={`Close ${trade.ticker}`} onClose={onClose}>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        ...
        <div className="md:col-span-2">
          ...
          <div className="flex items-center gap-2">
            <Button size="sm" variant={exitEmotion === "NEUTRAL" ? "default" : "outline"} onClick={() => setExitEmotion("NEUTRAL")}>
              Neutral
            </Button>
            <Button size="sm" variant={exitEmotion === "BIASED" ? "default" : "outline"} onClick={() => setExitEmotion("BIASED")}>
              Biased
            </Button>
          </div>
          <div className="mt-2">
...
/**
 * Render once under /app (e.g. inside your App outlet component).
 */
export function AppModals() {
  ...
  return (
    <>
      {s.showNew && (
        <NewTradeDialog
          onClose={() => s.setShowNew(false)}
          onCreated={(t) => {
            (async () => {
              try {
                const fresh = await apiFetchOpenTrades();
                s.setOpenTrades(Array.isArray(fresh) ? (fresh as Trade[]) : []);
              } catch {
                s.setOpenTrades((prev) => {
                  const nonMocks = prev.filter((x) => !(typeof x.id === "string" && /^t\d+$/.test(String(x.id))));
                  return [t, ...nonMocks];
                });
              }
              void s.refreshDashboard();
            })();
          }}
          kpiError={s.kpiError}
          onGoRisk={() => navigate("/app/risk")}
          perTradeCap$={s.perTradeCap$}
          dailyRemaining$={s.dailyRemaining$}
          guardsEnabled={s.guardsEnabled}
        />
      )}
      ...
      {s.closing && (
        <CloseTradeDialog
          trade={s.closing}
          onClose={() => s.setClosing(null)}
          onClosed={(id) => {
            s.setOpenTrades((prev) => prev.filter((t) => t.id !== id));
            void s.refreshDashboard();
          }}
        />
      )}

      {s.editOpen && s.editing && (
        <TradeEditor
          mode="edit"
          initial={{
            id: s.editing.id,
            ticker: s.editing.ticker,
            side: s.editing.side,
            entryPrice: s.editing.entryPrice,
            stopLoss: s.editing.stopLoss ?? null,
            target: s.editing.target ?? null,
            size: s.editing.size,
            notes: s.editing.notes ?? "",
            strategyTags: s.editing.strategyTags ?? [],
          }}
          onSubmit={s.onSaveEdit}
          onClose={() => {
            s.setEditOpen(false);
            s.setEditing(null);
          }}
